...
def clasificar(texto_raw):
    ...
    if coords:
        ...
        return "Neuquén/Otros",f"{dist:.1f} km de Añelo"
...
def procesar_excel(uploaded_file):
    ...
    df[col_otro]   =df["_clasif"].apply(lambda x:"X" if x=="Neuquén/Otros"  else "")
    df.rename(columns={col_neuquen:"Neuquén",col_otro:"Neuquén/Otros"},inplace=True)
    df.drop(columns=["_clasif"],inplace=True)
    cant_a=(df[col_anelo]=="X").sum()
    cant_n=(df["Neuquén"]=="X").sum()
    cant_o=(df["Neuquén/Otros"]=="X").sum()
    total=len(df)
...
🔵 <b>Neuquén/Otros</b> — Todo lo que no entra en las categorías anteriores
</div>
""",unsafe_allow_html=True)
        ...
        try:
            ...
            <div class="metric-row">
                ...
                <div class="metric-card otros">
                    ...
                    <div class="label">Neuquén/Otros &nbsp;·&nbsp; {po:.1f}%</div>
                </div>
            </div>
            """,unsafe_allow_html=True)
            st.download_button(
